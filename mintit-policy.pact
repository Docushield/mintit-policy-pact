...
(module mintit-policy-staging-v9 GOVERNANCE
 ...
 (defun count-nfts-by-owner-in-collection:integer
  ...
  (defun transfer-minted-tokens (collection-name:string start-index:integer end-index:integer)
    (map 
      (transfer-minted-token collection-name)
      (enumerate start-index end-index)
    )
  )

  (defun transfer-all-minted-tokens (collection-name:string)
    (with-capability (ADMIN_ACCESS)
      (with-read nft-collection-table collection-name
        {'starting-index := starting-index, 'num-minted := num-minted}
        
        (map 
          (transfer-minted-token collection-name)
          (enumerate starting-index (+ starting-index num-minted))
        )
      )
    )  
  )
